...
const refresh = (courseId?: string) => {
  ...
  if (courseId) revalidatePath(`/carnet/cours/${courseId}`)
}
...
/**
 * Crée un cours. Depuis le 10/09/2026, la feuille de création peut lui donner
 * sa MATIÈRE (`subject_id`, migration 316) et l'allure qui va avec — icône et
 * couleur héritées de la matière (`lib/carnet/matiere`). Tout est facultatif :
 * un cours « sans matière » reste le livre violet d'avant.
 */
export async function createCourse(
  title?: string,
  options: { subjectId?: string | null; icon?: string; color?: string } = {},
): Promise<OkId> {
  ...
  const ligne: Record<string, unknown> = {
    owner_id: userId,
    title: normalizeTitle(title, 'Nouveau dossier'),
  }
  if (typeof options.subjectId === 'string' && options.subjectId.length > 0) {
    ligne.subject_id = options.subjectId
  }
  if (options.icon !== undefined) ligne.icon = normalizeCourseIcon(options.icon)
  if (options.color !== undefined) ligne.color = normalizeCourseColor(options.color)
...
export async function createQuestion(
  ...
  // L'origine « manuel » (357). Tant que la 357 dort, PostgREST refuse la
  // colonne inconnue : on réinsère sans elle plutôt que de refuser la question.
  let res = await supabase
    .from('carnet_questions')
    .insert({ ...ligne, source: 'manuel' })
    .select('id')
    .single()
  if (res.error && colonneInconnue(res.error)) {
    res = await supabase.from('carnet_questions').insert(ligne).select('id').single()
  }
  if (res.error || !res.data) {
    console.error('[carnet-cours] création de la question impossible:', res.error?.message)
    return fail
  }
  refresh(courseId)
