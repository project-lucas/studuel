...
/**
 * L'ONGLET RÉVISER, construit dès l'ouverture de l'app et gardé vivant,
 ...
 * `app/reviser/page.tsx` ne porte que le titre. Voir `lib/nav-tabs`
 * (`ongletVivant`) et `docs/latence.md`.
 */
export default async function OngletReviser() {
  ...
  const [
    ...
    toutLire<{ id: string; type: string; content: unknown }>((from, to) =>
      supabase
        ...
        .select('id, type, content, carnet_courses!inner(owner_id)')
  ...
  return (
    <div className="flex flex-col gap-3 pt-3 md:pt-0">
      ...
      {/* UN SEUL espace : le programme officiel. Le carnet, qui partageait cet
          ...
          le 15/09/2026 (`app/carnet`) — le bouton « Ma bibliothèque » (ex-« Mon carnet ») y
          mène. */}
