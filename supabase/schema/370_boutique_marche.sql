-- =============================================================================
-- 370 — LE MARCHÉ DE LA BOUTIQUE : un boost XP court et le boost TROPHÉES
--
...
-- devient le « Marché », une vitrine FIXE (elle ne tourne plus chaque lundi) :
--
--   · Boost XP       ('double-xp-2h')   — l'XP gagnée compte double pendant
--                     2 h. Même mécanique que la 368 (`double_xp_jusqua`), une
--                     durée courte, donc un prix bas : on le lance avant une
--                     séance de révision.
--   · Boost trophées ('trophees-x2-2h') — les trophées GAGNÉS comptent double
--                     pendant 2 h, en duel classé comme dans les jeux de salon.
--                     C'est apply_game_trophies (238) qui double, CÔTÉ SERVEUR :
--                     le client n'annonce que victoire ou défaite, il ne peut
--                     rien gonfler. Une défaite coûte le barème normal.
--   · (Le bloc « Fiche de révision » du Marché n'est qu'un lien vers le choix
--     de la matière : rien en base.)
--
-- Les anciennes offres (double XP 24 h / 3 j, gels, gemmes ×2) RESTENT en
-- base : un boost déjà acheté continue de valoir jusqu'à sa fin. Elles ne sont
-- simplement plus en vitrine, et l'action serveur (app/tresor/boutique-actions)
-- n'achète que ce que liste lib/boutique/offres.ts.
--
-- Dépend de 238 (apply_game_trophies, game_trophies, subject_peaks) et de 368
-- (user_wallet, boutique_offres, boutique_achats, acheter_offre).
-- Idempotente : ADD COLUMN IF NOT EXISTS, DROP/ADD de contrainte, seeds en
-- ON CONFLICT, CREATE OR REPLACE.
-- À exécuter à la main dans : Supabase Dashboard → SQL Editor → New query → Run.
-- =============================================================================

-- ─────────────────────────────────────── 1. l'état du boost trophées

ALTER TABLE public.user_wallet
  ADD COLUMN IF NOT EXISTS trophees_x2_jusqua TIMESTAMPTZ;

-- Rien à changer côté droits : la policy « user_wallet_select_own » (192)
-- laisse l'élève LIRE sa ligne, colonne neuve comprise ; seules les fonctions
-- definer ci-dessous écrivent.

-- ─────────────────────────────────────── 2. le catalogue du Marché

-- Le CHECK en ligne de la 368 porte le nom que Postgres lui a donné.
...
ALTER TABLE public.boutique_offres ADD CONSTRAINT boutique_offres_kind_check
  CHECK (kind IN ('double_xp', 'gel_serie', 'gemmes_x2', 'trophees_x2'));
...
-- `valeur` : la durée en heures.
INSERT INTO public.boutique_offres (id, kind, prix_gemmes, valeur) VALUES
  ('double-xp-2h',   'double_xp',   20, 2),
  ('trophees-x2-2h', 'trophees_x2', 25, 2)
...
-- La 368, plus le boost trophées. Chaque type a désormais SA branche : la 368
-- traitait « tout le reste » comme le week-end des gemmes ×2, ce qui aurait
-- appliqué un week-end à un boost trophées.
...
BEGIN
  ...
  ELSIF v_offre.kind = 'gemmes_x2' THEN
    ...
    -- celui en cours (inchangé depuis la 368).
...
-- La 238 à l'identique, plus UNE règle : tant que le boost trophées court, une
-- VICTOIRE rapporte le double de sa bande (20 au lieu de 10 en bande 0, 4 au
-- lieu de 2 en bande 8). La défaite n'est pas touchée. Le boost est lu ici, en
-- base, au moment du verdict — jamais annoncé par le client. La réponse dit
-- s'il a joué (`boostTrophees`), pour l'écran de fin.
...
BEGIN
  ...
  IF p_won THEN
    -- Le boost trophees (370) : lu en base, au moment du verdict. Pas de
    -- portefeuille, ou colonne a NULL : pas de boost.
...
-- La 238 n'accordait qu'aux élèves connectés sans retirer le droit par défaut
-- de PUBLIC : on ferme aussi anon (la fonction rendait déjà NULL sans session).
