// Espagnol — Seconde : LE PROGRAMME DE LANGUE (34 fiches).
//
// LE DÉFAUT. La page « Espagnol » d'un élève de Seconde s'ouvre sur TROIS fiches
// maison, héritées de la migration 220 et écrites pour la 2de, la 1re et la Tle
// à la fois : « Les temps du passé », « Ser, estar et les tournures
// essentielles », « Le monde hispanique aujourd'hui ». La Terminale a reçu son
// programme en 231 puis 244, la Première en 267 ; la Seconde est le dernier
// niveau du lycée à en être restée là. Un élève qui bloque sur la négation,
// l'enclise des pronoms, cuyo, l'apocope, le subjonctif ou la concordance ne
// trouve RIEN.
//
...
// La fiche culturelle « Le monde hispanique aujourd'hui » part donc aussi de la
// Seconde, pour la raison qui l'a fait partir de la Terminale en 244 et de la
// Première en 267 : une fiche unique qui prétend tenir tous les axes culturels
// d'une année n'est pas un chapitre du programme, c'est une ligne de plus qui
// rouvre le doute sur les autres.
//
// POURQUOI LE MÊME CONTENU QU'EN TERMINALE, IMPORTÉ ET NON RECOPIÉ. Même
// raisonnement qu'en 266 et 267 : les programmes de LV sont écrits pour le CYCLE
// TERMINAL (2de-1re-Tle), pas pour un niveau, et la grammaire espagnole de
// Seconde est celle de Terminale. Le générateur sait dupliquer un bloc sur
// plusieurs niveaux — mais pas ici : les migrations 231 et 267 sont DÉJÀ
// ÉCRITES, et ajouter '2de' aux niveaux de la première les ferait se régénérer
// différemment. On importe donc les 34 chapitres de `espagnol-tle.mjs` sans y
...
//
// ⚠️ Le slug reste `espagnol` (la matière existe depuis 008) et QUATRE modules
// le portent désormais (`espagnol-lycee.mjs` → 220, `espagnol-tle.mjs` → 231,
// `espagnol-1re.mjs` → 267, celui-ci → 287) : ne JAMAIS générer avec
// `--slugs espagnol`, qui les fusionnerait et réécrirait trois migrations.
// Toujours `--modules espagnol-2de`.
//
// ⚠️ LA 220 EST REJOUABLE : la recoller un jour ferait revenir les trois fiches
// maison sur les trois niveaux du lycée. C'est le prix de l'idempotence.
...
// même que celle de `espagnol-1re.mjs`.
const AXES = {
...
// ci-dessous suffit à la retirer du niveau 2de.
//
...
const chapitres = espagnolTle.blocs[0].chapitres.map((ch) => {
  ...
  if (!axe) throw new Error(`espagnol-2de : aucun chapitre de programme pour « ${ch.titre} »`)
  return { ...ch, axe }
})
...
export default {
  ...
  titreMigration: 'L’ESPAGNOL DE SECONDE, RENDU À SON PROGRAMME',

  motif: `LE DÉFAUT : l'espagnol de Seconde n'avait que les 3 fiches maison de la
...
COLUMN IF NOT EXISTS, comme dans les migrations 243 à 286 — la 234 n'a jamais été
exécutée telle quelle. Sans cette reprise, la migration échouerait sur « column
chapters.theme does not exist », les 3 anciennes fiches déjà supprimées et les 34
...
GRANT SELECT (theme) ON public.chapters TO authenticated;`,
    ...
    {
      raison: `Les 3 fiches maison de la 220 partent, au niveau 2de SEULEMENT.

LE REPÈRE EST theme IS NULL, PAS LE TITRE — même raison qu'en 266, 267, 276, 277
et 284 : « Le monde hispanique aujourd'hui » porte une apostrophe, et rien ne
...
les trois lignes voulues : elles datent de la 220, bien avant la colonne theme,
tandis que les 34 fiches neuves en portent un dès l'INSERT — le ménage tourne
AVANT les insertions et ne peut donc jamais mordre sur elles, ni au premier
passage ni au rejeu.
Le filtre level = '2de' est indispensable : l'espagnol existe sur six niveaux
(5e → Tle) et le collège porte lui aussi des chapitres sans theme.
...
à « Espagnol / 2de » par subject + grade_level, donc toujours tirables par le
