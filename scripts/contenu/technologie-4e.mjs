// Technologie — 4e : LE PROGRAMME DU CYCLE 4, IMPORTÉ DE LA 3e (23 fiches).
//
// LE DÉFAUT. La technologie de 4e n'avait que QUATRE chapitres hérités du premier
...
const chapitres = technologie3e.blocs[0].chapitres.map((ch) => {
  if (!ch.axe) throw new Error(`technologie-4e : aucun chapitre de programme pour « ${ch.titre} »`)
  return { ...ch }
})
...
export default {
  ...
  titreMigration: 'TECHNOLOGIE 4E — LE PROGRAMME DU CYCLE 4 (23 fiches)',

  motif: `CONSTAT : la technologie de 4e n'avait que 4 chapitres hérités du premier jeu de
...
PÉRIMÈTRE : le niveau 4e SEUL — le ménage est borné à level = '4e'.`,
...
GRANT SELECT (theme) ON public.chapters TO authenticated;`,
    ...
    {
      raison: `Les 4 chapitres hérités partent, au niveau 4e SEULEMENT.
...
Le filtre level = '4e' est indispensable : la technologie existe sur quatre
