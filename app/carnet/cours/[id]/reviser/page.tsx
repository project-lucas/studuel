  ...
  const [{ data: chapterRows }, { data: questionRows }, { data: tagRows }] =
    await Promise.all([
      ...
      supabase
        ...
        .eq('course_id', id),
      // Les étiquettes (migration 316). La RLS les borne aux questions de
      // l'élève, mais PAS à ce cours-ci : la lecture porte donc sur tout son
      // carnet, et sans pagination elle s'arrêtait à 1 000 liaisons — au-delà,
      // le filtre par étiquette de cet écran devenait incomplet en silence. La
      // table peut être absente : la lecture est isolée et son échec ne casse
      // rien.
      toutLire<{ question_id: string; tag_id: string }>((from, to) =>
        supabase
          ...
          .range(from, to)
          .returns<{ question_id: string; tag_id: string }[]>(),
      ),
    ])
