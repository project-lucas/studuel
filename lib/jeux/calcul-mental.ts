...
//
// Et chaque opération porte son ASTUCE (lib/jeux/calcul-astuces) : la méthode
// la plus rapide, écrite avec ses nombres. Le jeu ne corrige plus seulement,
// il montre comment on va vite — c'est ce que l'élève vient y chercher.
...
import { astucePour, astuceTexte, type OperationSpec } from '@/lib/jeux/calcul-astuces'
 ...
 * inversée, la retenue oubliée — les plus crédibles de tous. `spec` est la
 * forme de l'opération, dont l'astuce se déduit.
 */
type Operation = {
  key: string
  label: string
...
const add = (a: number, b: number): Operation => ({
  key: `${a}+${b}`,
  label: `${a} + ${b}`,
  answer: a + b,
  spec: { kind: 'add', a, b },
})
const sub = (a: number, b: number): Operation => ({
  key: `${a}-${b}`,
  label: `${a} − ${b}`,
  answer: a - b,
  spec: { kind: 'sub', a, b },
})
const mul = (a: number, b: number): Operation => ({
  key: `${a}x${b}`,
  label: `${a} × ${b}`,
  answer: a * b,
  spec: { kind: 'mul', a, b },
})
const pct = (p: number, n: number, traps: number[]): Operation => ({
  key: `${p}%${n}`,
  label: `${p} % de ${n}`,
  answer: (n * p) / 100,
  spec: { kind: 'pct', p, n },
...
function drawApprenti(rng: () => number): Operation {
  ...
  if (kind === 0) {
    ...
    // Le leurre de qui oublie la retenue.
    return { ...add(a, b), traps: [a + b - 10] }
  }
  if (kind === 1) {
  ...
  return mul(intBetween(rng, 2, 10), intBetween(rng, 2, 10))
}
...
function drawConfirme(rng: () => number): Operation {
  const kind = intBetween(rng, 0, 2)
  if (kind === 0) return add(intBetween(rng, 13, 89), intBetween(rng, 12, 89))
  if (kind === 1) {
    const a = intBetween(rng, 35, 99)
    const b = intBetween(rng, 11, a - 1)
    return sub(a, b)
  }
  return mul(intBetween(rng, 3, 12), intBetween(rng, 3, 12))
}
...
function drawExpert(rng: () => number): Operation {
  ...
  if (kind === 0) return add(intBetween(rng, 45, 199), intBetween(rng, 25, 99))
  if (kind === 1) {
  ...
  if (kind === 2) {
    ...
    return mul(a, intBetween(rng, 3, 12))
  }
...
function drawMaitre(rng: () => number): Operation {
  const kind = intBetween(rng, 0, 3)
  if (kind === 0) return add(intBetween(rng, 125, 899), intBetween(rng, 125, 899))
  if (kind === 1) return mul(intBetween(rng, 12, 25), intBetween(rng, 12, 19))
  if (kind === 2) {
    const n = intBetween(rng, 1, 12) * 20
    const p = [15, 20, 30, 60][intBetween(rng, 0, 3)] as number
    return pct(p, n, [(n * (p + 10)) / 100, n / 10])
  }
...
function drawAuDela(rng: () => number, tier: number): Operation {
  ...
  const kind = intBetween(rng, 0, 3)
  if (kind === 0) {
    return add(intBetween(rng, 200 * g, 400 + 900 * g), intBetween(rng, 150 * g, 300 + 700 * g))
  }
  if (kind === 1) {
    return mul(intBetween(rng, 12 + 3 * g, 25 + 6 * g), intBetween(rng, 12, 19 + 3 * g))
  }
  if (kind === 2) {
...
export function buildCalculMentalPool(
  ...
  while (questions.length < count && guard < count * 8) {
    ...
    const astuce = astucePour(o.spec)
    questions.push({
      id: `jx-calc-${o.key}`,
      prompt: `Combien font ${o.label} ?`,
      options,
      correctIndex: options.indexOf(String(o.answer)),
      explanation: `${o.label} = ${o.answer}.`,
      subject: 'Maths',
      ...(astuce ? { astuce: astuceTexte(astuce) } : {}),
    })
  }
  return questions
}
